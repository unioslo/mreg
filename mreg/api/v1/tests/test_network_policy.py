...
class NetworkPolicyTestCase(ParametrizedTestCase, MregAPITestCase):
    ...
    def test_create_community_network_missing_policy_attribute(self):
        ...
        self.assertEqual(
            res.json()["errors"][0]["detail"], "Network does not have a policy. The policy must have the following attributes: ['isolated']"
        )
        ...
        self.assertEqual(
            res.json()["errors"][0]["detail"], "Network policy 'empty_policy' is missing the following required attributes: ['isolated']"
        )
