...
class ExamplePermissionTestWithContextManager(MregAPITestCase):
    ...
    def test_user_gains_permission_mid_test(self):
        ...
        NetGroupRegexPermission.objects.create(group="example_group", range="10.0.0.0/24", regex=r".*\.example\.org$")
        ...
        response = client.post("/api/v1/hosts/", {"name": "test.example.org", "ipaddress": "10.0.0.1"})
        ...
        with disable_policy_parity():
            ...
            response = client.post("/api/v1/hosts/", {"name": "test2.example.org", "ipaddress": "10.0.0.2"})
...
class ExampleComplexPermissionTest(MregAPITestCase):
    ...
    def test_permission_escalation_and_deescalation(self):
        ...
        NetGroupRegexPermission.objects.create(group="group1", range="10.0.0.0/24", regex=r".*\.example\.org$")
        NetGroupRegexPermission.objects.create(group="group2", range="10.0.1.0/24", regex=r".*\.example\.com$")
        ...
        with disable_policy_parity():
            ...
            response = client.post("/api/v1/hosts/", {"name": "test.example.org", "ipaddress": "10.0.0.2"})
            self.assertEqual(response.status_code, 201)
        ...
        with disable_policy_parity():
            ...
            response = client.post("/api/v1/hosts/", {"name": "test.example.com", "ipaddress": "10.0.1.1"})
            self.assertEqual(response.status_code, 201)
            client.delete(response["Location"])

            response = client.post("/api/v1/hosts/", {"name": "test.example.org", "ipaddress": "10.0.0.3"})
            self.assertEqual(response.status_code, 403)
